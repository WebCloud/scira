                    ...
                    reason_search: tool({
                        ...
                        execute: async ({ topic, depth }: { topic: string; depth: 'basic' | 'advanced' }) => {
                            ...
                            const { object: researchPlan } = await generateObject({
                                model:
                                    model === 'scira-sonnet'
                                        ? scira.languageModel('scira-sonnet')
                                        : scira.languageModel('scira-default'),
                                temperature: 0,
                                schema: z.object({
                                    search_queries: z
                                        .array(
                            ...
                            for (const step of stepIds.searchSteps) {
                                ...
                                if (step.type === 'web' || step.type === 'academic' || step.type === 'both') {
                    ...
                    memory_manager: tool({
                        ...
                        parameters: z.object({
                            ...
                            filters: z
                                .record(z.any())
                                .optional()
                                .describe('Additional filters for search/get operations'),
                        }),
                        execute: async ({
                            action,
                            content,
                            ...
                            try {
                                switch (action) {
                                    ...
                                    case 'delete': {
                                        if (!memoryId && !query) {
                                            throw new Error(
                                                'Either memoryId or query is required for delete operation',
                                            );
                                        }
