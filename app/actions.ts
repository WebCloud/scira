...
export async function suggestQuestions(history: any[]) {
    ...
    const { object } = await generateObject({
        model: ollama('qwen2.5-coder:14b', { structuredOutputs: true, simulateStreaming: true }),
        temperature: 0,
        maxTokens: 300,
        topP: 0.3,
        topK: 7,
...
const groupTools = {
    web: ['web_search', 'reason_search', 'datetime'] as const,
    buddy: [] as const,
    academic: ['code_interpreter', 'datetime'] as const,
